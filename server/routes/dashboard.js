...
router.get('/', async (req, res) => {
  ...
  const count = async (sql, params = []) => (await db.get(sql, ...params)).c;
  const pf = partyFilter ? ' AND party_id=?' : '';

  const partiesTotal = await count("SELECT COUNT(*) AS c FROM parties WHERE status IN ('planejada','ativa')");
  const partiesAtivas = await count("SELECT COUNT(*) AS c FROM parties WHERE status = 'ativa'");

  const statusCounts = {};
  for (const s of statuses) {
    statusCounts[s.key] = await count('SELECT COUNT(*) AS c FROM orders WHERE status=?' + pf, [s.key].concat(partyFilter ? [partyFilter] : []));
  }
  const abertos = await count(
  ...
  const occupiedTables = await count(occupiedBase, partyFilter ? [partyFilter, 'ocupada'] : ['ocupada']);
  const tableTotal = partyFilter
    ? await count('SELECT COUNT(*) AS c FROM party_tables pt WHERE pt.party_id = ?', [partyFilter])
    : await count('SELECT COUNT(*) AS c FROM party_tables pt');
  ...
  if (readyStage) {
    const prep = await db.get(
      ...
      partyFilter ? [readyStage, partyFilter] : [readyStage]
    );
